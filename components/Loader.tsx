...
export default function Loader() {
  ...
  useEffect(() => {
    if (window.sessionStorage.getItem("kayi-loader-seen") === "true") {
      setVisible(false);
      return;
    }

    const timer = window.setTimeout(() => {
      window.sessionStorage.setItem("kayi-loader-seen", "true");
      setVisible(false);
    }, 950);
    ...
    <AnimatePresence>
      {visible ? (
        <motion.div
          ...
          initial={{ opacity: 1 }}
          exit={{ y: "-100%", transition: { duration: 0.65, ease: [0.83, 0, 0.17, 1] } }}
        >
          <motion.div
          ...
          <div className="mt-8 h-px w-56 overflow-hidden bg-ivory/10">
            <motion.div
              ...
              initial={{ x: "-100%" }}
              animate={{ x: "100%" }}
              transition={{ duration: 0.78, delay: 0.12, ease: [0.65, 0, 0.35, 1] }}
            />
          </div>
        </motion.div>
      ) : null}
